...
const createScene = async () => {
    ...
    //Glow post-process (makes emissive parts bloom)
    const glowLayer = new BABYLON.GlowLayer("glow", scene);
    glowLayer.intensity = 0.5; // tweak 0.5 - 3
    // --- Hemi light (soft fill) ---
    const hemi = new BABYLON.HemisphericLight("hemi", new BABYLON.Vector3(0, 1, 0), scene);
    hemi.intensity = 0.55;

    // AMbient occlusion
// ✅ Ambient Occlusion (SSAO2) — since GTAO2 isn't in your build
const ssao = new BABYLON.SSAO2RenderingPipeline("ssao", scene, {
  ssaoRatio: 1, // AO resolution scale (0.5 = faster)
  blurRatio: 0.5  // blur resolution scale
});

// Quality / look tuning
ssao.radius = 2.4;          // size of occlusion (try 1.2–4)
ssao.totalStrength = 1.6;   // intensity (try 0.6–1.6)
ssao.expensiveBlur = true;  // smoother but heavier
ssao.maxZ = 150;            // far cutoff (tweak for your scene scale)
// Ambient occlusion shit
/* 
// ✅ Environment IBL (looks like glTF viewers)
scene.environmentTexture = BABYLON.CubeTexture.CreateFromPrefilteredData(
  "https://assets.babylonjs.com/environments/environmentSpecular.env",
  scene
);

// ✅ Skybox like a viewer
scene.createDefaultSkybox(scene.environmentTexture, true, 1000);

// AES tone mapping SHIT
const ipc = scene.imageProcessingConfiguration;
ipc.toneMappingEnabled = true;
ipc.toneMappingType = BABYLON.ImageProcessingConfiguration.TONEMAPPING_ACES;
ipc.exposure = 1.0;   // try 0.9–1.4
ipc.contrast = 1.0;   // try 1.0–1.2

// AES tone mapping SHIT END */

// Attach to your FPS camera
scene.postProcessRenderPipelineManager.attachCamerasToRenderPipeline("ssao", camera);


// Ambient occlusiojn end
    ...
    cubeResult.meshes.forEach(m => {
        if (m.getTotalVertices && m.getTotalVertices() > 0) {
            ...
            if (mat) {
                // Works for PBRMaterial and StandardMaterial
                mat.emissiveColor = new BABYLON.Color3(0.2, 0.8, 1.0); // cyan-ish glow color
            }
            //
        }
    });
    ...
    // --- LOAD MOSSY CUBE GLB end


    // metalTest start
    const metalTest = await BABYLON.SceneLoader.ImportMeshAsync(null, "models/", "metalTest.glb", scene);
    metalTest.meshes.forEach(m => {
        m.checkCollisions = true;
        m.isPickable = false;
        shadowGenerator.addShadowCaster(m, true);
    });

    //ADD test rock cube shit
    // rockCube start
    const rockCube = await BABYLON.SceneLoader.ImportMeshAsync(null, "models/", "rock.glb", scene);
    rockCube.meshes.forEach(m => {
        m.checkCollisions = true;
        m.isPickable = false;
        shadowGenerator.addShadowCaster(m, true);
    });
    // metalTest END
    // END test rock cube shit

    const casaBloque = await BABYLON.SceneLoader.ImportMeshAsync(null, "models/", "casaBloque.glb", scene);
    casaBloque.meshes.forEach(m => {
        m.checkCollisions = true;
        m.isPickable = false;
        shadowGenerator.addShadowCaster(m, true);
    });
